 ...
 */
type WAKitEventData = Partial<WAKitEventMap>

const BUFFERABLE_EVENT_SET = new Set<WAKitEvent>(BUFFERABLE_EVENT)

type WAKitBufferableEventEmitter = WAKitEventEmitter & {
	/** Use to process events in a batch */
	process(handler: (events: WAKitEventData) => void | Promise<void>): () => void
	/**
 ...
 */
export const makeEventBuffer = (logger: ILogger): WAKitBufferableEventEmitter => {
	...
	// take the generic event and fire it as a wakit event
	ev.on('event', (map: WAKitEventData) => {
		for (const event in map) {
			ev.emit(event, map[event as keyof WAKitEventMap])
		}
	})
	...
	return {
		process(handler) {
			const listener = async (map: WAKitEventData) => {
				await handler(map)
			}
		...
		emit<T extends WAKitEvent>(event: WAKitEvent, evData: WAKitEventMap[T]) {
			...
			if (event === 'messages.upsert') {
				const { type } = evData as WAKitEventMap['messages.upsert']
		...
		createBufferedFunction(work) {
			return async (...args) => {
				...
				try {
					...
					// Decrement atomically. Schedule flush only if we are the last
					// outstanding buffer AND no flush is already pending.
					// This eliminates the race where two concurrent callers both
					// observe bufferCount === 0 before either sets flushPendingTimeout.
...
function append<E extends BufferableEvent>(
	...
	switch (event) {
		...
		case 'contacts.update':
			const contactUpdates = eventData as WAKitEventMap['contacts.update']
		...
		case 'messages.upsert':
			const { messages, type } = eventData as WAKitEventMap['messages.upsert']
		...
		case 'messages.update':
			const msgUpdates = eventData as WAKitEventMap['messages.update']
		...
		case 'messages.delete':
			const deleteData = eventData as WAKitEventMap['messages.delete']
		...
		case 'messages.reaction':
			const reactions = eventData as WAKitEventMap['messages.reaction']
		...
		case 'message-receipt.update':
			const receipts = eventData as WAKitEventMap['message-receipt.update']
			...
			break
		case 'groups.update':
			const groupUpdates = eventData as WAKitEventMap['groups.update']
