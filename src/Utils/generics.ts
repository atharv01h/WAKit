...
const wakitVersion = [2, 3000, 1043857760]
import type {
	WAKitEventEmitter,
	WAKitEventMap,
	ConnectionState,
...
export function bindWaitForEvent<T extends keyof WAKitEventMap>(ev: WAKitEventEmitter, event: T) {
	return async (check: (u: WAKitEventMap[T]) => Promise<boolean | undefined>, timeoutMs?: number) => {
		let listener: (item: WAKitEventMap[T]) => void
...
export const bindWaitForConnectionUpdate = (ev: WAKitEventEmitter) => bindWaitForEvent(ev, 'connection.update')

/**
 * utility that fetches latest wakit version from the master branch.
 * Use to ensure your WA connection is always on the latest version
 */
export const fetchLatestWAKitVersion = async (options: RequestInit = {}) => {
	const URL = 'https://raw.githubusercontent.com/atharv01h/WAKit/master/src/Defaults/index.ts'
	try {
		...
		if (!response.ok) {
			throw new Boom(`Failed to fetch latest WAKit version: ${response.statusText}`, { statusCode: response.status })
		}
		...
		return {
			version: wakitVersion as WAVersion,
			isLatest: false,
			error
		}
	}
...
/**
 * Stream errors generally provide a reason, map that to a wakit DisconnectReason
