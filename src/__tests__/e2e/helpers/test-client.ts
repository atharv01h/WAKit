...
const uniquePushName = (prefix = 'wakit-e2e') => `${prefix}-${randomUUID()}`
...
const resolveConfig = (opts: TestClientOptions): ResolvedConfig => ({
	...
	authDir: opts.authDir ?? join(tmpdir(), `wakit-e2e-${randomUUID()}`),
...
const openConnection = async (
	...
	for (;;) {
		...
		// pairing-handoff close: WAKit terminates the noise socket once paired, we re-open
		config.logger.debug({ reason: result.reason }, 'reconnecting after pairing handoff')
	}
}

/**
 * WAKit socket wrapped with e2e ergonomics: QR autoscan via bartender admin,
...
export class TestClient {
	...
	async resolveTestGroup(name = 'WAKit Group Test'): Promise<string | undefined> {
	...
	waitForEvent<K extends keyof WAKitEventMap>(
		event: K,
		predicate: (data: WAKitEventMap[K]) => boolean,
		timeoutMs = DEFAULT_TIMEOUT_MS
	): Promise<WAKitEventMap[K]> {
		return waitWithTimeout<WAKitEventMap[K]>(
			emit => {
				const handler = (data: WAKitEventMap[K]) => {
