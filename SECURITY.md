...
**Do not file security issues as public GitHub issues or PRs.** A public report is itself an exploit broadcast — many downstream projects pull WAKit directly from `master` or from `github:atharv01h/WAKit`, so a vulnerable version is in the wild within minutes.

Use either of these private channels:

1. **Preferred:** [GitHub Security Advisories](https://github.com/atharv01h/WAKit/security/advisories/new) on this repository. This gives us a private workspace to coordinate the fix, draft a CVE, and notify downstream projects.
2. **Email:** **contact@atharvhatwar.com** — Atharv Hatwar, current maintainer. Use this if GitHub Advisories is unavailable, or if the report involves the maintainer's keys / accounts directly.
...
- Vulnerabilities in dependencies that are reachable through WAKit' public API
...
- Bugs that only affect availability of the local process (a malformed input that throws an exception in your handler is not a CVE; a malformed input that lets a remote party crash any WAKit client is)
- Issues in third-party WhatsApp clones or forks of WAKit
- Behavior of WhatsApp itself — we don't control the server; report those to Meta
- Missing rate limiting in *your* application — WAKit doesn't add rate limiting on top of WhatsApp's
...
A few notes for downstream users — these aren't vulnerabilities in WAKit, but they are common ways production systems leak:

- **`WAKit_auth_info/` is equivalent to a long-lived credential.** Store it like an SSH private key. Don't commit it. Don't put it in container images. Encrypt it at rest if you can.
...
- **Pin a release.** Pulling `github:atharv01h/WAKit` always grabs `master`. Use a tagged release in production.

For non-security questions, see the repository documentation.
